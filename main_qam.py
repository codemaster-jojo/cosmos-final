...
from model import Constellation, QAMDecoder, make_qam16_points
...
# Random start so learning is observable; use init="qam16" for square baseline.
constellation_model = Constellation(
    points=make_qam16_points(init="random"),
    M=16,
).to(device)
...
constellation_model = Constellation(points=make_qam16_points(init="qam16"), M=16)
...
with open("list_of_bits.txt", "r") as file:
    ...
    for line in file:
        ...
        symbol_indices = bits_to_symbol_indices(bits, 16)
